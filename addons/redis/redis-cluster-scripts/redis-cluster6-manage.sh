...
get_current_comp_nodes_for_scale_in() {
  ...
  declare -A curren_comp_ip_map
  if [ "$network_mode" != "advertised_svc" ]; then
    # get announce ip for current primary
    for pod_fqdn in $(echo $CURRENT_SHARD_POD_FQDN_LIST | tr ',' '\n'); do
      pod_ip=$(getent hosts "$pod_fqdn" | awk '{print $1; exit}')
      curren_comp_ip_map["$pod_ip"]="$pod_fqdn"
    done
  fi
  ...
  while read -r line; do
    ...
    if [[ "$network_mode" == "advertised_svc" ]]; then
      ...
      if [[ -n "${curren_comp_ip_map[$node_ip]}" ]]; then
        belong_current_comp=true
      fi
    fi
...
init_current_comp_default_nodes_for_scale_out() {
  ...
  handle_default_network_mode() {
    ...
    pod_ip=$(redis_config_get "$pod_fqdn" "$SERVICE_PORT" "$REDIS_DEFAULT_PASSWORD" "config get cluster-announce-ip" | sed -n '2p')
    if is_empty "$pod_ip"; then
      echo "Error: Failed to get pod ip for pod $pod_name: $pod_fqdn " >&2
      return 1
    fi

    categorize_scale_out_node_map "$pod_name" "$pod_ip:$SERVICE_PORT" "$pod_name_ordinal"
    return 0
  }
